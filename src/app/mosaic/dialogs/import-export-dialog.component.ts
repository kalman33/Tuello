...
export class ImportExportDialogComponent {
  ...
  export() {
    const json = this.storageService.exportConfig();
  ...
  onFileChange(event: Event) {
    ...
    reader.onload = async () => {
      try {
        await this.storageService.importConfig(reader.result as string);
        this.snackBar.open(this.translate.instant('mmn.mosaic.import.success'), '', { duration: 2000 });
