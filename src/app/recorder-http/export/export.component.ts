...
@Component({
  ...
  imports: [MatDialogTitle, MatDialogContent, NgClass, ExtendedModule, MatButton, MatIcon, MatDivider, MatDialogActions, MatDialogClose, MatSlideToggle, FormsModule, TranslatePipe]
})
export class ExportComponent {
  libFileName = 'Tuello-Lib-file.js';
  routeAnimationsElements = ROUTE_ANIMATIONS_ELEMENTS;
  // Mode verbeux de la librairie exportee : logs des bouchons trouves / non trouves dans la console.
  verboseLib = false;
    ...
    fetch(url)
      ...
      .then((txt) => {
        chrome.storage.local.get(['deepMockLevel'], (results) => {
          ...
          // Active (ou non) les logs de la librairie exportee. Fait avant l'injection des donnees
          // pour ne pas risquer de matcher le placeholder dans le json des bouchons.
          txt = txt.replace('###IMPORT_VERBOSE###', this.verboseLib ? 'true' : 'false');
          ...
          this.zone.run(() => {
            this.snackBar.open(this.translate.instant('mmn.recorder-http.export.saveAsLib.information.message'), this.translate.instant('mmn.spy-http.import.success.action'), { duration: 1000 });
          });
        });
      });
  }
