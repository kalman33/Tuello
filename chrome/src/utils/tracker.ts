  ...
  return new Promise((resolve, reject) => {
    if (tuelloTracks) {
      ...
      chrome.storage.local.get(['tuelloTracks'], results => {
        if (!chrome.runtime.lastError && results['tuelloTracks']) {
  ...
  chrome.storage.local.get(['tuelloTrackData'], results => {
    ...
    list.getEntries().filter(entry => {
      ...
      if (url.search) {
        ...
        track.querystring = JSON.parse('{"' + decodeURI(url.search.substring(1)).replace(/"/g, '\\"').replace(/&/g, '","').replace(/=/g, '":"') + '"}')
      } else {
        track.url = url.href;
      }
...
function clickListener(e) {
  ...
  if (!e.target?.id.includes('tuello')) {
